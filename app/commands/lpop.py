    ...
    async def execute(self):
        try:
            ...
            if not isinstance(self.cache[key], deque):
                self.writer.write(msg("ERR wrong type of value for 'lpop' command", RESPType.ERROR))
                await self.writer.drain()
                return
            ...
            for _ in range(min(sz, len(self.cache[key]))):
                popped_values.append(self.cache[key].popleft())
            ...
            self.writer.write(msg("ERR wrong number of arguments for 'lpop' command", RESPType.ERROR))
            await self.writer.drain()
            return